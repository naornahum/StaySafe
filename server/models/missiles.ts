...
export function insertMissiles() {
  const statement = completedDatabase.prepare(
    "INSERT INTO missiles (model,quantity,size,manufacturing_year,location_id,officer_id) VALUES(?,?,?,?,?,?)"
  );
...
export function getMissiles() {
  const statement = completedDatabase.prepare("SELECT * FROM missiles");
  const missiles = statement.all();
  return missiles;
}

export function getMissileByID(missile_id: string) {
  const statement = completedDatabase.prepare("SELECT * FROM missiles WHERE missile_id = ?");
  const missiles = statement.all(missile_id);
  return missiles;
}

export function createMissiles(missile: any) {
  const statement = completedDatabase.prepare(
    "INSERT INTO missiles (model,quantity,size,manufacturing_year,location_id,officer_id) VALUES(?,?,?,?,?,?)"
  );
...
export function updateMissile(missileId: any, missile: any) {
  const oldMissile = completedDatabase
    .prepare(`SELECT * FROM missiles WHERE missile_id = ${missileId} `)
    .get();
  const newMissile = { ...oldMissile, ...missile };

  const statement = completedDatabase.prepare(
    "UPDATE missiles SET model = ?,quantity = ?,size = ?,manufacturing_year = ?,location_id = ?,officer_id = ? WHERE missile_id = ?"
  );
  statement.run([
    newMissile.model,
    newMissile.quantity,
...
export function deleteMissileByID(missile_id: string) {
  const deleteStatement = completedDatabase.prepare(
    "DELETE FROM locations_history WHERE missile_id = ?"
  );
  deleteStatement.run(missile_id);

  const statement = completedDatabase.prepare("DELETE FROM missiles WHERE missile_id = ?");
