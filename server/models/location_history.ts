...
export function insertLocationsHistory() {
...
export function getLocationsHistory() {
  const statement = completedDatabase.prepare("SELECT * FROM locations_history");
  const locationHistory = statement.all();
  return locationHistory;
}

export function getLocationHistoryByID(location_history_id: string) {
  const statement = completedDatabase.prepare(
    "SELECT * FROM locations_history WHERE location_history_id = ?"
  );
  const locationHistory = statement.all(location_history_id);
  return locationHistory;
}

export function createLocationHistory(location_history: any) {
  const statement = completedDatabase.prepare(
    "INSERT INTO locations_history (arrival_date,departure_date,missile_id,location_id) VALUES(?,?,?,?)"
  );
...
export function deleteLocationHistoryByID(location_history_id: string) {
  ...
  const statement = completedDatabase.prepare(
    "DELETE FROM locations_history WHERE location_history_id = ?"
  );
