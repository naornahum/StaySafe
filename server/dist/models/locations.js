...
exports.deleteLocationByID = exports.updateLocation = exports.createLocation = exports.getLocationByID = exports.getLocations = exports.insertLocations = void 0;
const database_1 = require("../database");
function insertLocations() {
    const statement = database_1.completedDatabase.prepare("INSERT INTO locations (lat,lon,base_name,nearest_city) VALUES(?,?,?,?)");
...
function getLocations() {
    const statement = database_1.completedDatabase.prepare("SELECT * FROM locations");
    const locations = statement.all();
    return locations;
}
exports.getLocations = getLocations;
function getLocationByID(location_id) {
    const statement = database_1.completedDatabase.prepare("SELECT * FROM locations WHERE location_id = ?");
...
function updateLocation(locationId, location) {
    const oldLocation = database_1.completedDatabase
        .prepare(`SELECT * FROM locations WHERE location_id = ${locationId} `)
        .get();
    const newLocation = Object.assign(Object.assign({}, oldLocation), location);
    const statement = database_1.completedDatabase.prepare("UPDATE locations SET lat = ?, lon = ?, base_name = ?, nearest_city = ? WHERE location_id = ?");
    statement.run([
        newLocation.lat,
        newLocation.lon,
...
function deleteLocationByID(location_id) {
    const deleteLocationHistoryStatement = database_1.completedDatabase.prepare("DELETE FROM locations_history WHERE location_id = ?");
    deleteLocationHistoryStatement.run(location_id);
    const deleteMissileStatement = database_1.completedDatabase.prepare("DELETE FROM missiles WHERE location_id = ?");
    deleteMissileStatement.run(location_id);
    const location = getLocationByID(location_id);
    const statement = database_1.completedDatabase.prepare("DELETE FROM locations WHERE location_id = ?");
